...
import { beginUsage, endUsage } from './llm';
...
export async function runHunt(huntRow: HuntRow, deps: HuntDeps): Promise<void> {
  ...
  try {
    ...
    deps.hunts.completeHunt(huntRow.id, { addCostCents: endUsage().costCents });
    log('hunt.done', { hunt: huntRow.id, extracted: collected.length, shown: ranked.length });
  } catch (err) {
    const msg = message(err);
    deps.hunts.failHunt(huntRow.id, msg, { addCostCents: endUsage().costCents });
