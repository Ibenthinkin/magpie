...
- [x] Harden `pacing.ts` — strict per-source rate caps, conservative human-like delays for marketplaces. *Challenge detection (CAPTCHA/blocked-page signals) + per-source cooldown added to `src/browser/pacing.ts`; a detected challenge backs the source off and is reported up through the hunt (see `src/engine/hunt.ts` challenge reporting).*
...
- [x] Vision/screenshot fallback path wired for what the guided path can't reach (fallback only, not happy path). *`src/engine/visionExtract.ts` (screenshot + anchor list → `generateObject` on a vision-capable model, same schema/validation boundary as text extraction) + `src/engine/visionFallback.ts` (screenshot/anchor orchestration), wired into `src/engine/hunt.ts` for adapter errors and empty-result runs, gated behind opt-in `MAGPIE_VISION_FALLBACK_ENABLED` (`src/index.ts`). Unit-tested (`tests/unit/visionExtract.test.ts`, `tests/unit/visionFallback.test.ts`) and e2e-tested against a real Playwright page + local fixture with the LLM mocked (`tests/bun/e2e/vision-fallback-e2e.test.ts`); `scripts/smoke-vision.ts` lets Ben review real vision cost/quality by hand before flipping the flag on.*
- [x] Detection mitigations as needed — `channel:'chrome'` real install and/or headed-in-xvfb (decided empirically; see SPEC §15). *`BROWSER_CHANNEL` env knob added to `src/browser/session.ts` (`resolveLaunchOptions`) — unset keeps today's bundled-Chromium behavior, setting it (e.g. `chrome`) launches a real Chrome install instead. Empirical headed-in-xvfb decision left to Ben's homelab deployment (no Dockerfile yet — tracked under Cross-cutting/Deployment below).*
